...
def execute_logic(metadata_path):
    logger.info("Executing Control: storage_buckets_acl_full_control_AllUsers")
    try: 
        all_buckets = get_sos_buckets()
        if not all_buckets.sos_buckets_usage: 
            logger.info("No buckets found. Skipping Control storage_buckets_acl_full_control_AllUsers...")
            return
        ...
        for bucket in all_buckets.sos_buckets_usage:
            for acl in bucket.acl:
                 if acl.permission == "FULL_CONTROL" and acl.grantee.uri and "AllUsers" in acl.grantee.uri:
