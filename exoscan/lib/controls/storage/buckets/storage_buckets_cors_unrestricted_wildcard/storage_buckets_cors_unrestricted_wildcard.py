...
def execute_logic(metadata_path):
    logger.info("Executing Control: storage_buckets_cors_unrestricted_wildcard")
    try: 
        all_buckets = get_sos_buckets()
        if not all_buckets.sos_buckets_usage: 
            logger.info("No buckets found. Skipping Control storage_buckets_cors_unrestricted_wildcard...")
            return
