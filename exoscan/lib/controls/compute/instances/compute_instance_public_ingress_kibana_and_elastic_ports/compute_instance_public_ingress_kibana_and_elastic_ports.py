...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_instance_public_ingress_kibana_and_elastic_ports")
    try: 
        all_instances = get_instances()
        if not all_instances.instances: 
            logger.info("No instances found. Skipping Control compute_instance_public_ingress_kibana_and_elastic_ports...")
            return
