import sys
from exoscan.lib.controls.compute.sks.inventory import get_sks
from exoscan.lib.controls.models import Finding
from log_conf.logger import logger

def execute_logic(metadata_path):
    logger.info("Executing Control: compute_sks_auto_update_disabled")
    try: 
        all_cluster = get_sks()
        if not all_cluster.sks_cluster: 
            logger.info("No sks-cluster found. Skipping Control compute_sks_auto_update_disabled...")
            return
