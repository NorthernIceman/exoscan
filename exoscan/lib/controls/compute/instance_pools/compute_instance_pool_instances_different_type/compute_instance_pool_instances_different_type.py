...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_instance_pool_instances_different_type")
    try: 
        all_instance_pools = get_instance_pools()
        if not all_instance_pools.instance_pools: 
            logger.info("No instance-pools found. Skipping Control compute_instance_pool_instances_different_type...")
            return
        ...
        for instance_pool in all_instance_pools.instance_pools:
            if not instance_pool.instances:
                break
            target_type = instance_pool.instance_type.id
            for instance in instance_pool.instances:
                instance_to_check = get_instances(instance.id)
                if instance_to_check.instance_type.id != target_type: 
