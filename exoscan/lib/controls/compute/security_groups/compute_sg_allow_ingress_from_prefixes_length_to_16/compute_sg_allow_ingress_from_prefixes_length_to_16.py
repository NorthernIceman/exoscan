...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_sg_allow_ingress_from_prefixes_length_to_16")
    try: 
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_sg_allow_ingress_from_prefixes_length_to_16...")
            return
        findings = []
        prefix_threshold = "16"
        found_sg = []
        for sg in all_sg.security_groups:
            ...
            for rule in sg.rules:
                if rule.network != None and rule.flow_direction == "ingress" and not (rule.network.startswith("10.") or rule.network.startswith("172.") or rule.network.startswith("192.168.")): 
                    ...
                    if prefix <= prefix_threshold and prefix != "0": #not 0 cause there is another control doing this
