...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_security_groups_allow_ingress_from_quad_zero")
    try: 
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_security_groups_allow_ingress_from_quad_zero...")
            return
        findings = []
        ...
        for sg in all_sg.security_groups:
            ...
            for rule in sg.rules:
                if rule.network == "0.0.0.0/0" or "::0/0":
