...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_sg_allow_ingress_from_internet_to_database_ports")
    try: 
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_sg_allow_ingress_from_internet_to_database_ports...")
            return
        findings = []
        ports = [27017, 27018, 7199, 9160, 8888, 9092, 11211, 3306, 1521, 2483, 5432, 6379, 1433, 1434]
        found_sg = []
