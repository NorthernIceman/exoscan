...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_security_groups_allow_ingress_from_all_udp_ports")
    try: 
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_security_groups_allow_ingress_from_all_udp_ports...")
            return
        findings = []
        ...
        for sg in all_sg.security_groups:
            ...
            for rule in sg.rules:
                if rule.start_port == 1 and rule.end_port == 65535 and rule.protocol == "udp":
