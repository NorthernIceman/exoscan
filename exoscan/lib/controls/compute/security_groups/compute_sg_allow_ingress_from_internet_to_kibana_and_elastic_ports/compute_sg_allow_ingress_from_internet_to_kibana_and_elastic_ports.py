...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_sg_allow_ingress_from_internet_to_kibana_and_elastic_ports")
    try: 
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_sg_allow_ingress_from_internet_to_kibana_and_elastic_ports...")
            return
        findings = []
