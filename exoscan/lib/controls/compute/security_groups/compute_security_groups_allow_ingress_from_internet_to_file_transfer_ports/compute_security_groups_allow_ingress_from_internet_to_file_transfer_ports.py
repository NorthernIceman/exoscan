...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_security_groups_allow_ingress_from_internet_to_file_transfer_ports")
    try: 
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_security_groups_allow_ingress_from_internet_to_file_transfer_ports...")
            return
        findings = []
        risky_ports = [20, 21]
        found_sg = []
