...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_security_groups_unused_group")
    try: 
        all_instances = get_instances()
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_security_groups_unused_group...")
            return
        findings = []
        unused_sg = []
