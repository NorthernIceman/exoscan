...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_sg_allow_ingress_from_internet_to_kerberos_ldap_ports")
    try: 
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_sg_allow_ingress_from_internet_to_kerberos_ldap_ports...")
            return
        findings = []
        ports = [88, 464, 749, 750, 389, 636]
        found_sg = []
