...
def execute_logic(metadata_path):
    logger.info("Executing Control: compute_sg_allow_ingress_from_all_ports")
    try: 
        all_sg = get_security_groups()
        if not all_sg.security_groups: 
            logger.info("No Security-Groups found. Skipping Control compute_sg_allow_ingress_from_all_ports...")
            return
        findings = []
        found_sg = []
            ...
            if sg.rules:
                for rule in sg.rules:
                    if rule.start_port == 1 and rule.end_port == 65535 and rule.protocol in ["tcp", "udp"] and rule.flow_direction == "ingress":
                        found_rules.append(f"(Rule-ID: {rule.id}, Protocol: {rule.protocol}))")
                if found_rules:
                    rules_str = "\n    ".join(found_rules)
